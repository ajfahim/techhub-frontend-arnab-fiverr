...
function Offer() {
  ...
  const handleDelete = async (offerId) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`http://localhost:4000/api/offer/deleteOffer/${offerId}`, {
          method: 'DELETE',
        });
  ...
  const handleCopy = async (offerId) => {
    const confirmDelete = window.confirm("Are you sure you want to copy this offer?");
    
    if (confirmDelete) {
      try {
        const response = await fetch(`http://localhost:4000/api/offer/copyOffer/${offerId}`, {
          method: 'POST',
        });
              ...
              <div className="flex flex-col justify-around h-full">
                ...
                >
                  <div className={`h-6 w-6  rounded-full`} style={
                    {
                      backgroundColor:(offer.recievedOffers.length === 0)?"red":"green"
                    }
                  }></div>
                ...
                <div className="flex justify-start gap-4 mt-4">
                  <button className="bg-primary-color text-white p-3 text-2xl rounded-full hover:bg-hover-color" onClick={()=>handleEdit(offer._id)}>
                    <FaEdit />
                  </button>
                  <button className="bg-primary-color text-white p-3 text-2xl rounded-full hover:bg-hover-color" onClick={()=>handleCopy(offer._id)}>
                    <FaRegCopy />
                  </button>
                  <button className="bg-primary-color text-white p-3 text-2xl rounded-full hover:bg-hover-color" onClick={()=>handleDelete(offer._id)}>
                    <RiDeleteBin6Line />
                  </button>
                </div>
              </div>
